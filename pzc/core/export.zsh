...
then
  ...
  export PZC_CMAKE_C_COMPILER_LAUNCHER="${PZC_CCACHE_BIN}"
else
  export PZC_CMAKE_C_COMPILER_LAUNCHER="null"
fi
...
then
  ...
  then
    ...
    export PZC_CMAKE_LINKER_TYPE="MOLD"
...
then
  ...
  PZC_CMAKE_GENERATOR="Unix Makefiles"
fi
...
then
  function y() {
  	...
  	command ${PZC_YAZI_BIN} "$@" --cwd-file="$tmp"
